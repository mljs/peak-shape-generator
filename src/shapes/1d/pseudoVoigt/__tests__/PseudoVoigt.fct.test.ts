...
import { PseudoVoigt, getPseudoVoigtFactor } from '../PseudoVoigt.ts';

test('mu equal 0, pure lorentzian', () => {
...
test('mu equal 1', () => {
  ...
  expect(pseudoVoigt.fct(0.1)).toBeCloseTo(0.5, 2);
  expect(pseudoVoigt.fct(0.1)).toBeCloseTo(pseudoVoigt.fct(-0.1), 2);
  expect(pseudoVoigt.fct(0.15)).toBeCloseTo(gaussianFct(0.15, 0.2), 2);
});

test('mu equal 0.5 or 0.2', () => {
